...
// Validate a "Directed To" value
export function validateDirectedTo(directedTo: DirectedTo): boolean {
  if (directedTo.type === "ministry") {
    return Boolean(directedTo.ministryId && getMinistryById(directedTo.ministryId));
  }
  if (directedTo.type === "governorate") {
    return Boolean(directedTo.governorateId && getGovernorateById(directedTo.governorateId));
  }
  if (directedTo.type === "center") {
    return Boolean(
      directedTo.governorateId &&
      directedTo.centerId &&
      getCenterById(directedTo.governorateId, directedTo.centerId)
    );
  }
  return false;
}
